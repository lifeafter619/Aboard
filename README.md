...
## 最近更新 (Recent Updates)
- **无限白板模式**：新增无限白板模式，提供无边界的自由创作体验，支持与分页模式切换。无限模式下支持自由缩放（0.01x - 1000x）和全内容区域导出。
...
- **无限白板**：
  - 无边界的创作空间，自由拖拽移动
  - 画布始终填满窗口，不受浏览器窗口大小影响
  - 自由缩放：取消最小/最大缩放限制，支持 0.01x - 1000x 缩放
  - 智能导出：自动计算内容边界，导出包含所有内容的矩形区域
- **分页模式**：
  - 支持多页管理，适合课堂演示和教学
...
│       ├── infinite-canvas-manager.js # 无限画布管理模块
