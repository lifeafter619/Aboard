...
### 🎲 随机点名器
- **快速点名**：输入名单（每行一个），点击开始随机滚动
- **视觉反馈**：滚动时快速切换名字，停止时高亮显示结果
- **数据持久化**：自动保存输入的名单，刷新页面不丢失
- **简单易用**：界面简洁，支持拖动，适合课堂互动

### 🖼️ 图片插入功能
- **插入图片**：支持从本地上传图片插入到画布
- **自由操作**：支持移动、缩放、旋转插入的图片
- **右键菜单**：右键点击图片可进行删除操作
- **层级管理**：图片作为独立对象，支持在其上方进行批注
...
**更多**~~~~我打算做小功能，目前放了“时间”、“计时”、“计分板”、“点名器”和“插入图片”五个小功能，都能独立设置调整，您可以自己探索：）
